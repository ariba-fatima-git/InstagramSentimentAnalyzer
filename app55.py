...
def roberta_sentiment(hf_token: str, texts: list) -> pd.DataFrame:
    ...
    for start in range(0, len(texts), batch_size):
        ...
        try:
            ...
            outputs = resp.json()
            # HF returns list of lists when input is a list
            if isinstance(outputs, dict) and "error" in outputs:
                raise ValueError(outputs["error"])
            ...
            # On error for this batch, fill with neutral placeholders
            for _ in batch:
                results.append({
                    "sentiment": "Neutral", "confidence": 0.0,
                    "score_pos": 0.0, "score_neu": 1.0, "score_neg": 0.0,
                })
...
def run_sentiment(_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if hf_token:
        with st.spinner("🤖 Running XLM-RoBERTa sentiment via Hugging Face API..."):
            sent_df = roberta_sentiment(hf_token, texts)
        sent_df = patch_cultural_sentiment(sent_df, texts)
    else:
        st.info("💡 HF_API_TOKEN not set — using TextBlob as fallback. Add HF_API_TOKEN to Streamlit secrets for multilingual RoBERTa.")
        try:
